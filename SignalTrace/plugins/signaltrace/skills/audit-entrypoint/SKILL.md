...
The script is the authority for robots and bot-directive decisions, request request budgets, concurrency, timeouts, redirect handling, body limits, URL deduplication, evidence caching, scope comparison, finding and opportunity deduplication, severity, and final serialization. Its governor is the only component allowed to execute curl. Always run `improvement-opportunity-audit` against the cached target representation; its recommendations are non-findings and cannot start requests. Do not perform side-channel network requests around the governor.
