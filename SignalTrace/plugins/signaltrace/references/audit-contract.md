...
Auth-gated paths such as `/account` and its descendants, `/login`, `/signin`, `/cart`, `/checkout`, and `/wishlist` are excluded from content-comparison role sampling. They remain recorded as skipped coverage, and their absence does not fill a role with an anonymous thin page. Public pages with thin content continue through the normal destination and detail checks.
